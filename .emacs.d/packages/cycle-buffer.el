...
;;
;; If I remove this statement, or change (switch-to-buffer (current-buffer))
;, in cycle-buffer to:
...
(defvar cycle-buffer-filter
  '((not (eq (aref (buffer-name) 0) ? )) ; " buffer"
    (not (member (buffer-name)          ; uninteresting buffers
                 '("lispdir.dat" "*reportmail*" ".newsrc-dribble")))
    (not (string-match "^TAGS" (buffer-name)))
...
A prefix argument specifies the DISTANCE to skip, negative moves back."
  ...
  (let ((time (current-time)) list last buf)
	...
	(progn
	  ...
	  ;; _XX (switch-to-buffer (current-buffer))
      ;; force record_buffer (current-buffer)
      (setq tmpbuf (current-buffer))
      (switch-to-buffer (other-buffer) t)
      (switch-to-buffer tmpbuf)
...
(defun cycle-buffer-shorten-name (s)
  ...
  (let (len prefix suffix)
    (if (string-match "ftp \\(anonymous@\\|ftp@\\)" s) ; cut off this part
        (setq s (concat (substring s 0 (match-beginning 1))
